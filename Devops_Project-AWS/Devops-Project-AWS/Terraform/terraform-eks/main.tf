...
resource "aws_eks_node_group" "node-ec2" {
  ...
  update_config {
    max_unavailable = try(each.value.update_configt.max_unavailable, 1)
  }
...
locals {
  oidc = "oidc.eks.your-region.amazonaws.com/id/your-cluster-id" # Update to match your OIDC provider URL
}
...
resource "aws_eks_addon" "addons" {
  ...
  # resolve_conflicts = "OVERWRITE" # Uncomment if supported in your provider version
}
